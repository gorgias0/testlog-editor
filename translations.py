TRANSLATIONS = {
    "sv": {
        "File": "Arkiv",
        "Edit": "Redigera",
        "Format": "Format",
        "Insert": "Infoga",
        "View": "Visa",
        "Tools": "Verktyg",
        "Language": "Språk",
        "New": "Ny",
        "Open...": "Öppna...",
        "Open Workspace...": "Öppna arbetsyta...",
        "Save": "Spara",
        "Save As...": "Spara som...",
        "Save Text As": "Spara text som",
        "Save Text A As...": "Spara text A som...",
        "Save Text B As...": "Spara text B som...",
        "Save Both As...": "Spara båda som...",
        "Save Text {label} As": "Spara text {label} som",
        "Save Both As": "Spara båda som",
        "Export as Markdown...": "Exportera som Markdown...",
        "Export as Text...": "Exportera som text...",
        "Export as PDF...": "Exportera som PDF...",
        "Quit": "Avsluta",
        "Undo": "Ångra",
        "Redo": "Gör om",
        "Copy Line": "Kopiera rad",
        "Cut Line": "Klipp ut rad",
        "Duplicate Line/Block": "Duplicera rad/block",
        "Move Line/Block Up": "Flytta rad/block upp",
        "Move Line/Block Down": "Flytta rad/block ned",
        "Bold": "Fetstil",
        "Italic": "Kursiv",
        "Underline": "Understruken",
        "Inline Code": "Inline-kod",
        "Code Block": "Kodblock",
        "Heading 1": "Rubrik 1",
        "Heading 2": "Rubrik 2",
        "Heading 3": "Rubrik 3",
        "Heading 4": "Rubrik 4",
        "Bullet List": "Lista",
        "Numbered List": "Numrerad lista",
        "Blockquote": "Blockquote",
        "Horizontal Rule": "Horisontell linje",
        "Insert Date": "Infoga datum",
        "Insert Time": "Infoga tid",
        "Insert Date & Time": "Infoga datum och tid",
        "Insert Link...": "Infoga länk...",
        "Insert Image...": "Infoga bild...",
        "Indentation": "Indrag",
        "2 Spaces": "2 mellanslag",
        "4 Spaces": "4 mellanslag",
        "Tabs": "Tabbar",
        "Date": "Datum",
        "Time": "Tid",
        "Text:": "Text:",
        "URL:": "URL:",
        "Alt text:": "Alt-text:",
        "Path or URL:": "Sökväg eller URL:",
        "Browse...": "Bläddra...",
        "Copy image into note": "Kopiera bild till anteckningen",
        "Select image": "Välj bild",
        "Image Files (*.png *.jpg *.jpeg *.gif *.bmp *.webp *.svg)": "Bildfiler (*.png *.jpg *.jpeg *.gif *.bmp *.webp *.svg)",
        "Could not copy image into note": "Kunde inte kopiera bilden till anteckningen",
        "Formatting": "Formatering",
        "+ New File": "+ Ny fil",
        "+ Folder": "+ Mapp",
        "Sorting": "Sortering",
        "Name (A-Z)": "Namn (A-Ö)",
        "Last Modified": "Senast ändrad",
        "Created": "Skapad",
        "Write Markdown here...": "Skriv Markdown här...",
        "Workspace opened: {path}": "Arbetsyta öppnad: {path}",
        "Select workspace": "Välj arbetsyta",
        "Select workspace first": "Välj arbetsyta först",
        "New file": "Ny fil",
        "Filename (without .testlog):": "Filnamn (utan .testlog):",
        "New folder": "Ny mapp",
        "Folder name:": "Mappnamn:",
        "Open testlog": "Öppna testlog",
        "TestLog Files (*.testlog)": "TestLog Files (*.testlog)",
        "Save testlog": "Spara testlog",
        "Markdown Files (*.md)": "Markdown-filer (*.md)",
        "Text Files (*.txt)": "Textfiler (*.txt)",
        "JSON Files (*.json)": "JSON-filer (*.json)",
        "HTML Files (*.html *.htm)": "HTML-filer (*.html *.htm)",
        "XML Files (*.xml)": "XML-filer (*.xml)",
        "CSV Files (*.csv)": "CSV-filer (*.csv)",
        "All Files (*)": "Alla filer (*)",
        "PDF Files (*.pdf)": "PDF Files (*.pdf)",
        "Export as Markdown": "Exportera som Markdown",
        "Export as Text": "Exportera som text",
        "Export as PDF": "Exportera som PDF",
        "Markdown exported": "Markdown exporterad",
        "Text exported": "Text exporterad",
        "Text saved": "Text sparad",
        "PDF export failed": "PDF export misslyckades",
        "PDF exported": "PDF exporterad",
        "Autosaved": "Autosparad",
        "Bold (Ctrl+B)": "Fetstil (Ctrl+B)",
        "Italic (Ctrl+I)": "Kursiv (Ctrl+I)",
        "Underline tooltip": "Understruken",
        "Inline code (`)": "Inline-kod (`)",
        "Code block (Ctrl+Shift+K)": "Kodblock (Ctrl+Shift+K)",
        "Heading 1 (Ctrl+1)": "Rubrik 1 (Ctrl+1)",
        "Heading 2 (Ctrl+2)": "Rubrik 2 (Ctrl+2)",
        "Heading 3 (Ctrl+3)": "Rubrik 3 (Ctrl+3)",
        "Heading 4 (Ctrl+4)": "Rubrik 4 (Ctrl+4)",
        "Insert date (Ctrl+Alt+D)": "Infoga datum (Ctrl+Alt+D)",
        "TestLog Editor": "TestLog Editor",
        "English": "Engelska",
        "Swedish": "Svenska",
        "Text Tool": "Textverktyg",
        "New Tab": "Ny flik",
        "Close Tab": "Stäng flik",
        "Rename Tab": "Ändra namn",
        "Close All Except This": "Stäng alla utom denna",
        "Scratch {number}": "Kladd {number}",
        "Diff": "Diff",
        "Diff...": "Diff...",
        "Status": "Status",
        "Todo": "Att göra",
        "Doing": "Pågår",
        "Done": "Klar",
        "Previous": "Föregående",
        "Next": "Nästa",
        "Format JSON A": "Formatera JSON A",
        "Format JSON B": "Formatera JSON B",
        "Format HTML A": "Formatera HTML A",
        "Format HTML B": "Formatera HTML B",
        "Ignore Whitespace": "Ignorera blanksteg",
        "Ignore Blank Lines": "Ignorera tomma rader",
        "No Changes": "Inga ändringar",
        "Change {current} of {total}": "Ändring {current} av {total}",
        "Send Current Text to Diff A": "Skicka aktuell text till Diff A",
        "Send Current Text to Diff B": "Skicka aktuell text till Diff B",
        "Text A": "Text A",
        "Text B": "Text B",
        "Show/hide preview (Ctrl+Shift+P)": "Visa/dölj förhandsgranskning (Ctrl+Shift+P)",
        "Split View": "Split-vy",
        "Toggle editor/preview (Ctrl+E)": "Växla editor/förhandsgranskning (Ctrl+E)",
        "Toggle editor/preview (Ctrl+E or F11)": "Växla editor/förhandsgranskning (Ctrl+E eller F11)",
        "Paste text here...": "Klistra in text här...",
        "Characters: {with_ws} | Without whitespace: {without_ws} | Cursor: {cursor_pos}": "Tecken: {with_ws} | Utan blanksteg: {without_ws} | Markör: {cursor_pos}",
        "Editor Count: {with_ws} | No ws: {without_ws} | Selected: {sel_with_ws} | Selected no ws: {sel_without_ws}":
            "Editor: {with_ws} | Utan blanksteg: {without_ws} | Markerat: {sel_with_ws} | Markerat utan blanksteg: {sel_without_ws}",
        "Generate Lorem": "Generera Lorem",
        "Counter String": "Räknarsträng",
        "Counter String Length": "Längd",
        "Transform": "Transformera",
        "Close": "Stäng",
        "To Base64": "Till Base64",
        "From Base64": "Från Base64",
        "To URL": "Till URL",
        "From URL": "Från URL",
        "Format JSON": "Formatera JSON",
        "Format HTML": "Formatera HTML",
        "Guess Markdown From Plain Text": "Gissa Markdown från vanlig text",
        "Invalid Base64": "Fel: ogiltig Base64-sträng",
        "Invalid JSON: {error}": "Fel: ogiltig JSON – {error}",
        "Best-effort JSON formatting applied: {error}": "Bästa möjliga JSON-formatering tillämpad: {error}",
        "UUID": "UUID",
        "Count": "Antal",
        "Testdata": "Testdata",
        "Generate Swedish Testdata": "Generera svensk testdata",
        "Sweden": "Sverige",
        "Norway": "Norge",
        "Denmark": "Danmark",
        "Finland": "Finland",
        "Iceland": "Island",
        "United Kingdom": "Storbritannien",
        "United States": "USA",
        "Special Characters": "Specialtecken",
        "Copy All": "Kopiera allt",
        "Clear": "Rensa",
        "OK": "OK",
        "Pinned Files": "Fästa filer",
        "Rename": "Byt namn",
        "Delete": "Ta bort",
        "Pin to Top": "Fäst högst upp",
        "Unpin": "Ta bort fästning",
        "Move To...": "Flytta till...",
        "Archive": "Arkivera",
        "Delete Folder {filename}? This cannot be undone.": "Ta bort mappen {filename}? Detta går inte att ångra.",
        "Delete {filename}? This cannot be undone.": "Ta bort {filename}? Detta går inte att ångra.",
        "Confirm Delete": "Bekräfta borttagning",
        "Move Item": "Flytta objekt",
        "Image copied": "Bild kopierad",
        "Copy": "Kopiera",
        "Copy Link": "Kopiera länk",
        "Open Link": "Öppna länk",
        "Copy Image": "Kopiera bild",
        "Copied": "✓ Kopierat",
        "Font Size": "Fontstorlek",
        "Light Mode": "Ljust läge",
        "Dark Mode": "Mörkt läge",
        "Search Files...": "Sök filer...",
        "Search Document...": "Sök i dokument...",
        "No Matches": "Inga träffar",
    }
}
